...
export function generateLaplace(){
    ...
    console.log("laplace function params test:"+contr+','+eps+','+ amt);

    // calculate b
    var b = Math.floor(contr/eps);
...
export function genKeyComb(params){
    ...
    {   
        ...
        {   
            ...
            sk = document.getElementById("subKey"+i).value.replace(/\s/g, '').split(",").map(Number);
            console.log("sk"+i);
            console.log(sk);
...
export function scaling(metrics, skId){
    ...
    budgetPM = Math.floor(contr/metrics);
    ...
    {
        var sKeys =  document.getElementById("subKey"+skId).value.replace(/\s/g, '').split(",").map(Number);
...
export function configSubKeys() {
    ...
    for( var i=1; i<=noSubKeys; i++){
         tbody += '<h4> Add sub key ' + i + ' key components, separated by comma ",": </h4>';
...
// generate randome purchase count
function generateRandP(value, index, array){
    ...
    var max = maxCnt;
    var min = 0;
    dataset[index][dataset[index].length] = Math.floor(Math.random() * (max - min + 1)) + min;
...
function calculateCnt(value, index, array){
    ...
    var currCnt = 0;
    var sKeys =  document.getElementById("subKey"+(parseInt(index)+1)).value.replace(/\s/g, '').split(",").map(Number);
    console.log("sKeys: "+sKeys);
...
function generateRandA(value, index, array){
    // agg purchase amt = a random no between purchase count (aka per purchase amt is 1) and purchase count * max amount;
    ...
    // generate rnadom vbalue between min and max with scaling factor for amount
...
function generateVal(index, ind, scale){
    ...
    // noise%
    var val = (dataset[index][ind*1+3] - dataset[index][ind*1])/dataset[index][ind*1];
    if(isFinite(val))
        dataset[index][ind*1+4] =  val;
    else
        dataset[index][ind*1+4] =0;
}
...
function generateTable(data) {
    ...
    downloadButton.setAttribute("onClick","download_table_as_csv('"+tid+"')");
