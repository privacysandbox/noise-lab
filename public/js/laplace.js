...
function simulatePerMetric(mainDiv, keyCombinations, metricsNo, metric) {
    ...
    const scalingFactor =  getIsUseScalingFromDom() ? getScalingFactorForMetric(metric, metricsNo, CONTRIBUTION_BUDGET) : 1
    const keyCombinationString = getKeyCombinationString(keyCombinations.names)

    console.log('!!SCALING FACTOR!!'+scalingFactor)
    const max = calculateMaximumCount(getFrequencyValue(), getDailyValue(), metric.maxValue)
    console.log(max)
    ...
    for (let i = 0; i < keyCombinations.combinations.length; i++) {
        const noise = getRandomLaplacianNoise(CONTRIBUTION_BUDGET, getEpsilonFromDom())
        const randCount = Math.floor(Math.random() * (max - min) + min)
        const noisePercentage = calculateNoisePercentage(noise, randCount * scalingFactor + noise)
    ...
    averageNoisePercentage = averageNoisePercentage / (keyCombinations.combinations.length)
    ...
    displayAdvancedReports(mainDiv, simulationReport, metric.name, scalingFactor, keyCombinationString)
}
...
export function triggerSimulation() {
    ...
    if (isGranular()) {
        ...
        for (let i = 0; i < allCombs.length; i++) {
            ...
            keyCombList.push({
                ...
                combinations: generateKeyCombinationArray(allCombs[i].combinations),
            })
        }
    ...
    metrics.forEach(element => {
        for (let i = 0; i < keyCombList.length; i++) {
            simulatePerMetric(simulationDiv, keyCombList[i], metrics.length, element)
        }
    })
